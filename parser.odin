...
parse_program :: proc(p: ^Parser) -> []^Ast_Node {
	...
	for {
		...
		if t.kind == .Import_Keyword {
			...
			// }
			// append(&stmts, module)
			for s in parse_program(&import_parser) do append(&stmts, s)
			continue
		}
...
precedence :: proc(op: Token_Kind) -> int {
	...
	case .Plus, .Minus:
		return 20
	case .DoubleLesser, .DoubleGreater:
		return 15
	case .Ampersand:
		return 10
	case .Caret:
	...
	case .Lesser, .Greater, .GreaterOrEqual, .LesserOrEqual, .DoubleEqual, .NotEqual:
		return 5
	case .DoublePipe, .DoubleAmpersand:
		return 4
...
parse_expression :: proc(
	...
	case .Minus, .Bang, .Ampersand, .Star, .Tilde:
		// Negative values, negation, address_of, dereference, bitwise NOT
